...
st.title("Chapter 9: Morphological Image Processing")
st.write("Xử lý ảnh hình thái")

st.markdown(
    f"""<style>
    .stApp {{
    background-image: url("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRS3o2Hds2OJoEVzKKSqIcPcJxKItlHHzwRl-CY_5BR6uDPzq8zK0XRIIIcqanc74ETvoo&usqp=CAU");
...
if uploaded_file is not None:
    ...
    option = st.selectbox(
        ...
        ('Không', 'Erosion', 'Dilation', 'OpeningClosing', 'Boundary', 'HoleFill',
    ...
    if option == 'Không':
        ...
        imgout = Erosion(image)
        st.image(imgout, caption='Ảnh sau khi xử lý', use_column_width=True)
    elif option == 'Dilation':
        imgout = Dilation(image)
        st.image(imgout, caption='Ảnh sau khi xử lý', use_column_width=True)
    elif option == 'OpeningClosing':
        imgout = OpeningClosing(image)
        st.image(imgout, caption='Ảnh sau khi xử lý', use_column_width=True)
    elif option == 'Boundary':
        imgout = Boundary(image)
        st.image(imgout, caption='Ảnh sau khi xử lý', use_column_width=True)
    elif option == 'HoleFill':
        imgout = HoleFill(image)
        st.image(imgout, caption='Ảnh sau khi xử lý', use_column_width=True)
    elif option == 'MyConnectedComponent':
        imgout = MyConnectedComponent(image)
        st.image(imgout, caption='Ảnh sau khi xử lý', use_column_width=True)
    elif option == 'ConnectedComponent':
        imgout = ConnectedComponent(image)
        st.image(imgout, caption='Ảnh sau khi xử lý', use_column_width=True)
    elif option == 'CountRice':
        imgout = CountRice(image)
        st.image(imgout, caption='Ảnh sau khi xử lý', use_column_width=True)